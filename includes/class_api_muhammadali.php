...
class Muhammadali{
	...
	private $muhammadali_url = "http://api.muhammadali-store.com:3100/";
	...
	private function call_curl($type,$url,$parms){	
		...
		$curl_url = $this->muhammadali_url.$url;
        ...
        if ($status==0) return ['error'=>1, 'message' => 'It was not possible to connect with the muhammadali server. Try again later.'];

		if ($status > 400){
                if ($status==401) return ['error'=>401, 'message' => 'Unauthorized access. Check if your token was filled in correctly or contact muhammadali support.'];
